...
const T = {
  en: {
    ...
    noContacts:'No contacts',settings:'Settings',profile:'Profile',voiceVideo:'Voice & Video',appearance:'Appearance',
    ...
    soundIncoming:'Incoming call sound',soundAccept:'Accept sound',soundReject:'Reject sound',soundMessage:'Message sound',
    testSound:'Test',customSound:'Custom URL'
  },
  ru: {
    messenger:'Мессенджер',search:'Поиск...',favorites:'Избранное',savedMessages:'Сохранённые сообщения',
    noContacts:'Нет контактов',settings:'Настройки',profile:'Профиль',voiceVideo:'Голос и видео',appearance:'Внешний вид',
    ...
    soundIncoming:'Звук входящего звонка',soundAccept:'Звук принятия',soundReject:'Звук отклонения',soundMessage:'Звук сообщения',
    testSound:'Тест',customSound:'Ссылка на звук'
  }
};
...
/* Sound Settings */
let soundSettings = {
  incoming: localStorage.getItem('sound_incoming') || 'https://res.cloudinary.com/dgg0ph1bx/video/upload/v1776503025/redmi-note-6-call_cip9il.mp3',
  accept: localStorage.getItem('sound_accept') || 'https://res.cloudinary.com/dgg0ph1bx/video/upload/v1776503024/picked-up-the-phone_ecvhjj.mp3',
  reject: localStorage.getItem('sound_reject') || 'https://res.cloudinary.com/dgg0ph1bx/video/upload/v1776503025/hung-up-on-the-rotary-phone_fkjqht.mp3',
  message: localStorage.getItem('sound_message') || 'https://res.cloudinary.com/dgg0ph1bx/video/upload/v1776503024/xiaomi-notice_vs9vod.mp3'
};
...
function playSound(type){
  const audio = type === 'incoming' ? soundIncomingCall : 
                type === 'accept' ? soundAcceptCall :
                type === 'reject' ? soundRejectCall : soundMessage;
  const url = soundSettings[type];
  if(!url) return;
  audio.src = url;
  audio.currentTime = 0;
...
const favoritesChat=$('#favoritesChat'),usersList=$('#usersList');
...
/* Appearance Panel Elements */
const appearanceOverlay=$('#appearanceOverlay'),appearancePanel=$('#appearancePanel'),appearanceBackBtn=$('#appearanceBackBtn');
const appearanceThemeSelect=$('#appearanceThemeSelect'),appearanceLangSelect=$('#appearanceLangSelect');
const soundIncomingInput=$('#soundIncomingInput'),soundAcceptInput=$('#soundAcceptInput');
const soundRejectInput=$('#soundRejectInput'),soundMessageInput=$('#soundMessageInput');
const testSoundIncoming=$('#testSoundIncoming'),testSoundAccept=$('#testSoundAccept');
const testSoundReject=$('#testSoundReject'),testSoundMessage=$('#testSoundMessage');
const saveSoundsBtn=$('#saveSoundsBtn');
const soundIncomingCall=$('#soundIncomingCall'),soundAcceptCall=$('#soundAcceptCall');
const soundRejectCall=$('#soundRejectCall'),soundMessage=$('#soundMessage');
const openAppearance=$('#openAppearance');
...
auth.onAuthStateChanged(async user=>{
  if(user){
    ...
    if(!profile){
      state.profile = {
        nickname: 'TestUser_' + user.uid.slice(0,6),
        tag: 'TEST' + Math.random().toString(36).substring(2,7).toUpperCase(),
        avatarUrl: '',
        bannerUrl: '',
        about: '🎉 Testing Veyo Messenger',
        musicUrl: '',
        musicCover: '',
        musicName: ''
      };
    ...
    auth.signInAnonymously().catch(err=>console.warn('Auto-login failed:', err));
  }
});
...
async function startMicTest(){
  try{
    micStream=await navigator.mediaDevices.getUserMedia({audio:{deviceId:micSelect.value?{exact:micSelect.value}:undefined,echoCancellation:false,noiseSuppression:false,autoGainControl:false}});
    ...
    // Connect to destination for loopback monitoring (echo)
    ...
    alert('Microphone test failed. Check browser permissions.');
  }
}
...
testSpeakerBtn.addEventListener('click', async ()=>{
  try {
    ...
    if(testAudio.setSinkId && speakerSelect.value) {
      await testAudio.setSinkId(speakerSelect.value);
    }
    await testAudio.play();
    testSpeakerBtn.classList.add('active');
    testAudio.onended = () => {
      testSpeakerBtn.classList.remove('active');
    };
  } catch(e) {
    console.warn('Speaker test failed', e);
    alert('Speaker test failed. Make sure audio file can be loaded.');
  }
});

/* ===== PROFILE MODAL ===== */
let pmPlaying=false;
profileBarAvatar.addEventListener('click',()=>openProfileModal(state.profile));

function openProfileModal(user){
  currentViewingUser = user;
  const p = user || state.profile;
...
pmVoiceCall.addEventListener('click', async () => {
  ...
  await startCall(currentViewingUser.uid, false);
});

pmVideoCall.addEventListener('click', async () => {
  closeProfileModalFn();
  if(!currentViewingUser) return;
  await startCall(currentViewingUser.uid, true);
});
...
function renderUsersList(users){
  usersList.innerHTML = '';
  if(!users.length){
    usersList.innerHTML = '<div style="padding:12px 16px;color:var(--text4);text-align:center;font-size:13px;">No users online</div>';
    return;
  }
  ...
  users.forEach(user => {
    ...
    info.innerHTML = `
      ...
      <div class="user-tag">${user.tag ? '@' + user.tag : 'No tag'}</div>
    `;
    info.style.cursor = 'pointer';
    info.addEventListener('click', () => openProfileModal(user));
    
    const actions = document.createElement('div');
    actions.className = 'user-actions';
    
    const callBtn = document.createElement('button');
    callBtn.className = 'user-action-btn';
    callBtn.textContent = '📞';
    callBtn.title = 'Call';
    ...
    videoBtn.className = 'user-action-btn';
...
function openAppearancePanel(){
  ...
  appearanceLangSelect.value = lang;
  soundIncomingInput.value = soundSettings.incoming;
  soundAcceptInput.value = soundSettings.accept;
  soundRejectInput.value = soundSettings.reject;
  soundMessageInput.value = soundSettings.message;
  setTimeout(()=>{
...
openAppearance.addEventListener('click', openAppearancePanel);
appearanceBackBtn.addEventListener('click', closeAppearancePanel);
appearanceOverlay.addEventListener('click', closeAppearancePanel);
...
appearanceLangSelect.addEventListener('change', () => {
  lang = appearanceLangSelect.value;
...
testSoundIncoming.addEventListener('click', () => testSound(soundIncomingInput.value || soundSettings.incoming));
testSoundAccept.addEventListener('click', () => testSound(soundAcceptInput.value || soundSettings.accept));
testSoundReject.addEventListener('click', () => testSound(soundRejectInput.value || soundSettings.reject));
testSoundMessage.addEventListener('click', () => testSound(soundMessageInput.value || soundSettings.message));

saveSoundsBtn.addEventListener('click', () => {
  soundSettings.incoming = soundIncomingInput.value || soundSettings.incoming;
  soundSettings.accept = soundAcceptInput.value || soundSettings.accept;
  soundSettings.reject = soundRejectInput.value || soundSettings.reject;
  soundSettings.message = soundMessageInput.value || soundSettings.message;
  
  localStorage.setItem('sound_incoming', soundSettings.incoming);
  localStorage.setItem('sound_accept', soundSettings.accept);
  localStorage.setItem('sound_reject', soundSettings.reject);
  localStorage.setItem('sound_message', soundSettings.message);
  
  saveSoundsBtn.textContent = t('saved');
  saveSoundsBtn.classList.add('saved');
  setTimeout(() => {
    saveSoundsBtn.textContent = t('save');
    saveSoundsBtn.classList.remove('saved');
  }, 1500);
});
